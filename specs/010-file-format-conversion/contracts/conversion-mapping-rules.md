# Contract: Format ↔ Document Model Mapping Rules
...
composed. This file is mirrored into `src/modules/conversion/README.md` so
the rules live in the repository, not only in the spec folder.
...
Rule 8's refusal is the point: `user name` and `user-name` would both
sanitize to `user_name`, and writing one over the other would lose a field
silently. FR-009 requires data that cannot be represented to be refused with
an explanation, not dropped.
